...
function fillPrompt(template, req){
  ...
  const val = {
    ...
    attached: req.attachmentsText || "None."
  };
...
function buildRequest(text, ctx, cfg){
  ...
  req.prompt = fillPrompt(tpl, req) + (hasPicture(req) ? "" : NO_PICTURE);
...
/* The picture input of the prompt action cannot be left empty, so when
   nothing is attached it is handed one white pixel. A strong model reads the
   rule about that pixel and ignores it; a mini model reads the prompt, then
   meets an image as the last thing it sees, and answers the image - "that is
   a blank white square, try attaching it again" - to "hello". So the last
   words before the image say, plainly, that it is not there. */
const NO_PICTURE = "\n\n=== NO PICTURE THIS TIME ===\nNothing is attached to this message. " +
  "An image input may still come after this text: it is an empty placeholder the app always has to send. " +
  "Do not look at it, describe it, mention it or ask for a picture - answer only what they said.";
