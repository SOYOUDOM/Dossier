...
const ACTIONS = {
  ...
  draftEmail: { write:false, needs:["subject","body"],
    ...
    what:"Write an email and show it as a draft they can copy or open in " +
         "their mail app. Nothing is sent — Resolv cannot send mail and does " +
  ...
  addToBrief: { write:true, needs:["line"],
    args:{ line:TXT, section:STR },
    what:"Add one line of fact to their \"About my work\" brief (workspace.brief) - " +
         "a term and what it means, a server and what runs on it, who owns a " +
         "system, an environment. section is one of: Team, Systems, Servers, " +
         "People, Words. Plain fact, one line. Asks first." },
...
function checkAction(raw){
  ...
  if (!def)
    return { bad:"\"" + name + "\" is not something Resolv can do. " +
...
function validate(payload){
  const out = { say:"", ask:"", actions:[], refused:[], note:"", choices:[], thread:"" };
  let body = payload;
    ...
    try { body = JSON.parse(body); }
      ...
      if (!t) return { say:"", ask:"", actions:[], refused:[],
                       note:"The endpoint answered with nothing at all. A flow " +
                            "needs a Response action for Resolv to hear anything." };
  ...
  /* the running summary of the conversation, written by the model for its
     own next turn (workspace.thread) - never shown as the answer */
  out.thread = str(body.thread, 1500);
...
const ACTIONS_IN_FULL = ["needRecords", "find", "remember", "learn", "addToBrief", "recall", "saveRunbook",
...
function buildRequest(text, ctx, cfg){
  ...
  const req = {
    ...
    workspace: {
      ...
      /* THEIR OWN DESCRIPTION OF THEIR WORK - team, systems, servers,
         environments, owners, the words they use. Short, always sent, and
         the first thing to read: it is how the model knows their world. */
      brief: ctx.brief || "",
      /* WHAT THEY ARE DOING: the record open in front of them (whole),
         records selected, a timer running, the view, and today's activity.
         "this", "it", "the ticket" with nothing else to go on mean open. */
      focus: ctx.focus || undefined,
      /* THIS CONVERSATION SO FAR, summarised by the model itself on the
         turns before (summary), how many earlier messages are not in
         conversation[] any more (earlier), and whether a new summary is
         wanted with this reply (wanted). */
...
async function test(cfg, ctx){
  ...
  try {
    out = await ask("ping from Resolv — reply with say only",
  ...
  add("The reply is the shape Resolv expects",
