@echo off
rem ===========================================================================
rem  Dossier.bat - the one thing to double-click.
rem
rem  It starts Dossier and gets out of the way. No window stays open: Dossier
...
rem
rem  No SQL Server on this PC? It still starts, and Dossier keeps its records
...
rem
rem  USE
rem    Dossier.bat                        start it (asks for your records
rem                                       folder the first time, then remembers)
rem    Dossier.bat "D:\Work\Dossier"      start it on that folder
rem    Dossier.bat startup                start with Windows, quietly
rem    Dossier.bat startup off            stop doing that
rem
...
if not exist "%SRC%" (
  ...
  echo   This file has to sit in the Dossier folder, beside dossier.html.
  pause
  exit /b 9
)
...
if defined BUILD (
  ...
  if not errorlevel 1 (
    echo.
    echo   Dossier is running, and this copy of it is out of date.
    echo   Quit it from the icon by the clock - right-click, Quit Dossier -
...
rem ---- and go ---------------------------------------------------------------
rem  start, not call: this window closes now and Dossier carries on without it
if defined ARGS (
  start "" "%EXE%" "%ARGS%"
) else (
  start "" "%EXE%"
...
echo   Setting Dossier up - a few seconds, and only this once.
...
if errorlevel 1 (
  echo.
  echo   Dossier did not compile. That is a bug in the source, not in your
...
if /i "%WHAT%"=="off" (
  ...
  echo   Dossier will not start with Windows any more.
  exit /b 0
)
if not exist "%EXE%" (
  call :compile
...
if errorlevel 1 (
  echo   Could not set Dossier to start with Windows.
  exit /b 1
)
echo.
echo   Dossier will start with Windows - quietly, as an icon by the clock.
...
echo   Undo it:  Dossier.bat startup off
echo.
exit /b 0
